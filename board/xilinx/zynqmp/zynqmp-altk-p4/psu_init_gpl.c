...
#include <xil_io.h>
#include "psu_init_gpl_private.h"
...
// static void prog_reg(unsigned long addr, unsigned long mask,
//                      unsigned long shift,
//                      unsigned long value)
// {
//     int rdata = 0;
...
unsigned long psu_clock_init_data(void)
{
    ...
    * 6 bit divider
    *  PSU_CRL_APB_PL2_REF_CTRL_DIVISOR0                           0x29
    ...
    * This register controls this reference clock
    * (OFFSET, MASK, VALUE)      (0XFF5E00C8, 0x013F3F07U ,0x01012900U)
    */
    psu_mask_write(CRL_APB_PL2_REF_CTRL_OFFSET,
                   0x013F3F07U, 0x01012900U);
    /*##################################################################### */
...
unsigned long psu_mio_init_data(void)
{
    ...
    * (OFFSET, MASK, VALUE)      (0XFF180204, 0xFDFFE03FU ,0x50000000U)
    */
    psu_mask_write(IOU_SLCR_MIO_MST_TRI0_OFFSET,
                   0xFDFFE03FU, 0x50000000U);
    /*##################################################################### */
...
unsigned long psu_peripherals_init_data(void)
{
    ...
    *  PSU_GPIO_DIRM_1_DIRECTION_1                                 0x800

    * Direction mode (GPIO Bank1, MIO)
    * (OFFSET, MASK, VALUE)      (0XFF0A0244, 0x03FFFFFFU ,0x00000800U)
    */
    psu_mask_write(GPIO_DIRM_1_OFFSET, 0x03FFFFFFU, 0x00000800U);
    /*##################################################################### */
    ...
    *  PSU_GPIO_OEN_1_OP_ENABLE_1                                  0x800

    * Output enable (GPIO Bank1, MIO)
    * (OFFSET, MASK, VALUE)      (0XFF0A0248, 0x03FFFFFFU ,0x00000800U)
    */
    psu_mask_write(GPIO_OEN_1_OFFSET, 0x03FFFFFFU, 0x00000800U);
    /*##################################################################### */
...
unsigned long psu_ddr_phybringup_data(void)
{
    ...
    while ((pll_retry > 0) && (!pll_locked))
    {
        ...
        pll_locked = (Xil_In32(0xFD080030) & 0x80000000) >> 31; /*PGSR0*/
